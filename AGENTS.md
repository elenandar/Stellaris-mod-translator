...
- Текущий рабочий MVP-0 — Python CLI без обязательных внешних runtime-зависимостей
  на текущем Mac владельца. Rust остаётся возможным будущим целевым runtime,
  но не блокирует практический MVP-0.
...
Явное owner-задание MVP-0 от 26 июля 2026 года является отдельным разрешением
на ограниченный практический Python CLI: synthetic tests, loopback Ollama и
отдельный candidate-каталог. Оно не разрешает чтение private localisation без
consent, изменение active paths, launcher registration или массовую обработку.
AUTH-first процесс и PR №11 для MVP-0 superseded и не являются зависимостью.
...
Для ограниченной реализации MVP-0 уровень рассуждения `Medium` используется по
умолчанию. Ошибки реализации, тестов, форматтера, аргументов, текущая ветка и
повторный запуск проверок исправляются в рамках задачи. `BLOCKED` допустим
только при внешнем препятствии: отсутствующих правах/данных либо небезопасном
разрушительном решении, которое нельзя принять самостоятельно.
