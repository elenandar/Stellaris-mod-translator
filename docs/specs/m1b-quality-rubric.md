...
Executable analysis принимает closed enum `split` (`tuning`/`holdout`) и
...
candidate/profile/split/class. Synthetic-scope materializer создаёт exact row
multiset из frozen corpus/results/findings/HGT, но его capability подтверждает
только scope consistency. `synthetic_scope_*` и raw helpers всегда возвращают
`decision_grade_eligible=false`; raw CFA minimum также всегда false. Caller
split/UUID membership, relabel, subset, extra или изменённый row не создают
scope evidence.

Agreement, statistical и CFA production decision helpers не принимают caller
rows или synthetic-scope capability. Они получают exact rows только из
отдельного full decision admission после всех report/run/owner/executable gates
и принимают лишь `holdout`. M1B-0 document schema v4 такой admission не выдаёт.
Missing/unknown split и mixed-split scope fail closed; tuning никогда не
увеличивает holdout `n`, numerator или gate.
...
missing review успешного output. И synthetic diagnostic-scope materialization,
и будущая full-admission decision materialization учитывают любую no-output row,
включая controlled failure и no-attempt, как applicable failure для D1–D5, CFA
gate и editorial approval, поэтому она не может исчезнуть из quality
denominator. Synthetic результат остаётся diagnostic/ineligible, а production
decision требует отдельного full admission. No-output row не добавляется в
отдельный CFA event-rate denominator: отсутствие output не создаёт безопасную
exposure и уже fail closed в quality gates.
...
Synthetic agreement rows materialize только внутри HGT validator из уже
...
Synthetic-scope capability связывает exact-complete multiset, но разрешает лишь
split-marked diagnostic helper с no decision eligibility. Production gate
получает rows из отдельного full decision admission и принимает только
однородный holdout; M1B-0 не может выдать такой admission. Generic raw
agreement/statistical/CFA helpers принимают public synthetic math vectors,
всегда возвращают no decision eligibility (и no raw CFA minimum) и сами по себе
не являются live/selection evidence.
