...
`(candidate/profile, gate или dimension, primary stratum,
...
Executable analysis принимает closed `dimension_or_gate`: пять D1-D5 contract
...
сворачивает events по source generation внутри candidate/profile/class.
...
`(candidate/profile, risk class, source_generation_id)`: если в любом contained
...
Executable v3 `human_ground_truth` row содержит ровно `adjudicates`,
...
M1B-0 v3 report является только partial synthetic contract и всегда отклоняет
`complete_benchmark` кодом `PARTIAL_REPORT_CANNOT_BE_COMPLETE`. Будущий live
report требует отдельной owner-accepted schema/analysis definition; ни один v3
...
frozen initial records. Generic row helper принимает только public synthetic
math vectors и сам по себе не является live/selection evidence.
...
Unblinded review допустим только как `secondary_unblinded` и
не восстанавливает primary success.
...
zero-event units на candidate/profile/class; меньшее `n` не проходит gate.

Для proportions D2–D5 публикуются per-stratum source-generation
