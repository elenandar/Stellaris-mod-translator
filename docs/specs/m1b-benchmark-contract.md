...
- Версия proposal: `m1b-benchmark-contract-v5`; protocol generation `106`;
...
Trusted analysis component — `m1b-analysis-policy-v5`, generation `106`.
...
Для `m1b-benchmark-contract-v5` public bundle обязан содержать ровно `17`
...
Попытка завершить `complete_benchmark` в protocol v5 с document schema v4
...
Следующие operational defaults являются **proposal protocol v5 и ожидают owner
...
Каждая decision-grade statistical shape явно несёт closed `split` со значением
только `tuning` либо `holdout`. Это относится к agreement, D1–D5 и другим
statistical gates, CFA input/scope и HGT-derived agreement rows. Для каждого из
трёх decision entrypoints validator после проверки frozen corpus, results,
findings и HGT materialize-ит exact полный row multiset и выдаёт internal sealed
provenance для этого scope. Issuer сам повторно проверяет exact document schema,
trusted bundle/protocol, canonical frozen corpus, candidates, results, findings и
HGT; он не принимает уже подготовленные caller maps. Только реально выданный им
opaque object признаётся provenance, а самостоятельно сконструированный объект,
caller-supplied split, UUID membership, перестановка либо изменение
outcome/stratum/risk fail closed.
Missing/unknown split отклоняется; mixed-split input не pooling-уется и получает
controlled `STATISTICAL_SPLIT_MIXED`. Gate helpers принимают только exact trusted
`holdout`; tuning разрешён лишь в отдельном split-scoped diagnostic результате с
`decision_grade_eligible=false` и никогда не закрывает holdout minimum/verdict.
Raw agreement, statistical и CFA helpers также всегда diagnostic-only: они не
возвращают decision eligibility, а raw CFA не удовлетворяет decision minimum.
Поэтому `23 tuning + 23 holdout` не создают agreement `n=46`, а `100 tuning + 103
holdout` не создают CFA `n=203`. Internal seal фиксирует control flow текущего
validator-а, но не заменяет будущий external owner-accepted executable manifest;
`EXECUTABLE_IMPLEMENTATION_IDENTITY_UNPROVEN` остаётся blocker.
...
и declared coverage, но не создаёт success. Trusted statistical materialization
учитывает любую no-output row, включая controlled failure и no-attempt, как
applicable failure для D1–D5, CFA gate и editorial approval, поэтому её нельзя
скрыть исключением из quality denominator. В отдельный CFA event-rate denominator
она не входит: отсутствующий output не является безопасной exposure. Для любой
