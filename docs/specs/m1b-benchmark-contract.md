...
поля `winner`, `baseline` и `m1b_verdict` отсутствуют. Executable v4 schema
распознаёт две exact пары `benchmark_state`:
`(complete=false, report_kind=partial_synthetic_conformance)` и
`(complete=true, report_kind=complete_benchmark)`. Но M1B-0 v4 разрешает только
первую и безусловно отклоняет вторую как
`PARTIAL_REPORT_CANNOT_BE_COMPLETE`. Будущий live benchmark требует новой
owner-accepted report schema/generation, а не смены boolean в v4. Candidate
...
M1B-0 v4 содержит отдельный closed object `context_limit_binding`. Его exact
...
Для `m1b-benchmark-contract-v4` public bundle обязан содержать ровно `17`
...
Попытка завершить `complete_benchmark` в M1B-0 v4 заканчивается раньше кодом
...
Следующие operational defaults являются **proposal M1B-v4 и ожидают owner
...
statistical gates, CFA input/scope и HGT-derived agreement rows. Split выводится
из frozen sample provenance и не принимается как свободно переопределяемый
aggregate label. Missing/unknown split отклоняется; mixed-split input не
pooling-уется и получает controlled `STATISTICAL_SPLIT_MIXED`. Gate helpers
принимают только `holdout`; tuning разрешён лишь в отдельном split-scoped
diagnostic результате с `decision_grade_eligible=false` и никогда не закрывает
holdout minimum/verdict. Поэтому `23 tuning + 23 holdout` не создают agreement
`n=46`, а `100 tuning + 103 holdout` не создают CFA `n=203`.
...
Executable result v4 содержит только `accounting`, `attempt_index`,
...
`technical_conformance`, `terminal_status`. Sample v4 дополнительно bind-ит
...
- **complete future benchmark** — пара `report_kind=complete_benchmark`,
  `complete=true` зарезервирована, но v4 всегда возвращает
...
Каждый initial finding review v4 неизменно связывает exact
...
`not_evaluated` и полностью запрещает findings, human/model finding reviews и
`human_ground_truth` для этой row. Такой no-output result остаётся в denominator
и никогда не является technical либо human-quality pass. Для любой row каждое D2–D5 значение `human_pass`,
