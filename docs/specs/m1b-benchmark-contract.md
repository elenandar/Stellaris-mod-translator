...
- Версия proposal: `m1b-benchmark-contract-v1`; состояние definition:
...
Текущие executable IDs — `schema_version=m1b-synthetic-contract-v2` и fixture
manifest `m1b-synthetic-contract-cases-v2`. Любая другая version unsupported,
даже если проходит общий version regex.
...
поля `winner`, `baseline` и `m1b_verdict` отсутствуют. Executable v2 schema
структурно знает две exact пары `benchmark_state`:
`(complete=false, report_kind=partial_synthetic_conformance)` и
`(complete=true, report_kind=complete_benchmark)`. Canonical M1B-0 fixture
использует только первую. Вторая не означает, что benchmark уже допустим:
текущий trusted registry целиком имеет state `proposed`, поэтому структурно
полный report блокируется `OWNER_DECISION_REQUIRED` до отдельного
owner-accepted registry. Candidate labels не должны раскрывать модель
...
Synthetic validator читает только explicit path, не более `4 MiB`, и принимает
strict UTF-8 JSON без duplicate keys либо lone surrogates. Schema использует
только integers: `parse_float` отклоняет любой decimal/exponent token, а
`parse_constant` — NaN/Infinity до schema validation.
...
Для `m1b-benchmark-contract-v1` public bundle обязан содержать ровно `15`
...
Текущие `quality_rubric`/`analysis_policy` definitions фиксируют только
synthetic M1B-0 shape и состояние `owner_decision_required`; они не изображают
принятые numeric thresholds либо full live analysis plan. После принятия exact
numbers и full report fields создаются новые definition version/generation и
hashes; reuse текущего version с другими bytes запрещён.
...
Попытка завершить `complete_benchmark` с proposed entries заканчивается
...
| `output_schema` | result/atom/accounting/count fields, quality dimensions и закрытые technical/editorial/terminal/dimension enums |
...
Executable result v2 содержит только `accounting`, `attempt_index`,
`attempt_stage`, `candidate_id`, `corpus_generation`, `dimension_records`,
`editorial_status`, `experiment_lane`, `initial_result_id`, `observed_atoms`,
`profile_generation`, `profile_version`, `protocol_generation`, `result_id`,
`sample_id`, `technical_conformance`, `terminal_status`.
...
разрешила. Другие pairings invalid. Retry attempt при frozen `retry_count=0`
запрещён. Cold/warm performance observations считаются отдельно и не увеличивают
...
- **complete future benchmark** — executable v2 schema структурно знает
  `report_kind=complete_benchmark`, `complete=true`, но current proposed registry
  не разрешает завершить этот state. После отдельного owner acceptance exact
  cross-product обязательных candidate/sample/profile assignments должен
  совпасть с manifest, а каждая assignment иметь один primary result либо
  controlled terminal failure; иначе report отклоняется раньше gate decision.
...
а не подразумеваются успешными. Complete report с missing/extra assignment,
candidate без result, смешанными generations либо partial coverage invalid.
...
- Fallback row имеет только lane/stage `fallback`/`fallback`,
  ...
  row запрещена.
- При `fallback=false` запрещены как fallback rows, так и ненулевые aggregate
  fallback counters. Поэтому counters другого stage не могут изображать
  несуществующий repair/fallback.
- При `retry_limit=0` `retry_attempt_count=0`, а `model_call_count <= 1` в каждой
  row. Terminal `success` требует `model_call_count=1`; latency/memory observation
  count не может превышать model-call count.
- `terminal_failure_count=1` тогда и только тогда, когда
  `terminal_status=controlled_failure`; иначе он равен нулю. Terminal failure
  остаётся в assignment/quality denominator.
- Row-level равенства
...
- При `residency_status=not_probed` model-call, initial/repair/retry, cold/warm
  latency и memory observation counts равны нулю; partial results разных
  protocol/profile/corpus generations не агрегируются.
...
перегенерируется молча, остаётся в assigned-candidate denominator и получает
заранее заданный conservative primary failure: zero success для каждой
applicable D2–D5 и запрет candidate/stratum pass. Unblinded review может быть
только дополнительным secondary evidence и не заменяет ground truth либо этот
primary failure.
