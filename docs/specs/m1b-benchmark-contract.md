...
- Версия proposal: `m1b-benchmark-contract-v7`; protocol generation `108`;
...
Trusted analysis component — `m1b-analysis-policy-v6`, generation `108`.
...
поля `winner`, `baseline` и `m1b_verdict` отсутствуют. Executable document schema
v4
распознаёт две exact пары `benchmark_state`:
`(complete=false, report_kind=partial_synthetic_conformance)` и
`(complete=true, report_kind=complete_benchmark)`. Но document schema v4 в M1B-0
разрешает только
первую и безусловно отклоняет вторую как
`PARTIAL_REPORT_CANNOT_BE_COMPLETE`. Будущий live benchmark требует новой
owner-accepted report schema/generation, а не смены boolean в document schema
v4. Candidate
...
M1B-0 document schema v4 содержит отдельный closed object
`context_limit_binding`. Его exact
...
terminal failure в denominator и отсутствие findings, human/model finding
reviews и human ground truth. Тот же запрет content evidence действует для
`terminal_status=not_applicable`: это отсутствие model attempt, а не output,
пригодный для semantic/literary review. Unknown uppercase failure code не
становится допустимым transition.
...
Для `m1b-benchmark-contract-v7` public bundle обязан содержать ровно `17`
...
Попытка завершить `complete_benchmark` в protocol v7 с document schema v4
заканчивается раньше кодом
...
| `implementation_identity_policy` | non-circular future executable-manifest/runtime-TCB admission contract; не exact bytes или runtime state текущей реализации |
| `validator_policy` | input-size и signed-integer limits, strict JSON modes, closed/offline fixture schema, controlled public output fields и lifetime-bound non-owning/non-evicting synthetic-scope registry |
...
generation. Manifest schema v1 bind-ит только declared file bytes: он не
доказывает фактически использованный Python runtime, import graph, module globals,
invocation state, отсутствие monkeypatch/debugger/tracing hooks или code,
исполняемого в том же process. Эти поверхности вместе с validator, analysis
engine, materializer и runner/harness входят в same-process TCB. Защита от
hostile same-process code потребовала бы отдельной process/authority boundary и
не входит в M1B-0. Пока external record, verifier и runtime/invocation admission
не существуют, live observation запрещён.
...
Следующие operational defaults являются **proposal protocol v7 и ожидают owner
...
Analysis trust имеет два несовместимых уровня.

1. **Synthetic scope provenance.** M1B-0 materializer повторно проверяет exact
   schema, trusted proposal bundle/protocol, canonical frozen corpus, candidates,
   results, findings и HGT — только analysis-source subset документа — и
   фиксирует exact row multiset каждого agreement/statistical/CFA scope. Он не
   проверяет и не заменяет provider policy, request boundary, context binding,
   implementation identity, benchmark state, coverage, acceptance, aggregate
   accounting и execution admission. `synthetic_scope_*` helpers вычисляют
   denominator/status только как diagnostics и всегда сохраняют
   `decision_grade_eligible=false` и raw/synthetic CFA minimum=false.
2. **Full decision admission.** Production/live agreement, statistical и CFA
   entrypoints не принимают caller rows либо synthetic-scope token. Они получают
   exact scope rows только из отдельного admission после успешной полной
   validation всех report/run gates, owner-accepted definitions и admitted
   executable/runtime TCB. Document schema v4 всегда partial и M1B-0 такой token
...
Protocol v7 byte-bind-ит это требование в canonical
`benchmark_contract.synthetic_scope_lifetime`, а validator policy v7 — точные
weak-key/non-owning, non-eviction, no-back-reference и internal-token-release
обязательства в `validator_policy.synthetic_scope_registry`. Удаление либо
ослабление любого из этих полей меняет component hash и не проходит trusted
registry; одного ненормативного комментария реализации недостаточно.
...
pooling-уется и получает controlled `STATISTICAL_SPLIT_MIXED`. Поэтому `23
tuning + 23 holdout` не создают agreement `n=46`, а `100 tuning + 103 holdout`
не создают CFA `n=203`.

Python capability является nominal same-process misuse guard, а не opaque,
cryptographic или sandbox security boundary. Reflection над globals/closures,
monkeypatching, import hooks, debugger/tracing и любой другой same-process code
находятся внутри TCB. External executable manifest v1 сам по себе также не bind-ит
runtime/import/invocation state; `EXECUTABLE_IMPLEMENTATION_IDENTITY_UNPROVEN`
остаётся blocker.
...
Executable result schema v4 содержит только `accounting`, `attempt_index`,
...
`technical_conformance`, `terminal_status`. Sample schema v4 дополнительно bind-ит
...
разрешила. Другие pairings invalid. При frozen `retry_limit=0`
...
- **complete future benchmark** — пара `report_kind=complete_benchmark`,
  `complete=true` зарезервирована, но document schema v4 всегда возвращает
...
Каждая row имеет `terminal_status=not_applicable`,
`technical_conformance=not_observed`, пустые `observed_atoms`, D1–D5
`not_evaluated` и zero accounting, поскольку model attempt не выполнялся.
Aggregate technical success равен `0`. Это declared partial coverage, не
incomplete benchmark pass и не synthetic D1 conformance.
...
Для output, прошедшего primary blinding и пригодного для blinded content review,
критические категории получают две независимые initial human reviews от разных
людей; disagreement проходит отдельную human adjudication. Self-identifying
output является закрытым fail-closed исключением и primary reviews/HGT не
получает. Случайные UUIDv4
...
model attempt и допускает только `technical_conformance=not_observed`, пустые
`observed_atoms`, `blinding_status=not_observed`, mapping `0`, D1–D5/editorial
`not_evaluated` и zero accounting.
...
`unblinded` и `never_unblinded`. Critical finding reviewable output, прошедшего
primary blinding, имеет ровно две primary initial human reviews; self-identifying
output остаётся в secondary-only exception. Только при disagreement adjudicator
является третьим
distinct never-unblinded человеком, связывает ровно два frozen review IDs и
задаёт final closed outcome, не переписывая initial evidence.

HGT adjudication подчиняется тому же fail-closed linkage: одна adjudication row
ссылается ровно на два distinct существующих `initial` HGT IDs того же
`result_id`, той же dimension и текущей mapping generation; initial outcomes
обязаны конфликтовать, а reviewer adjudication является одним distinct третьим
human. Orphan/cross-result/cross-dimension/duplicate link, согласующиеся initials,
лишний adjudicator либо любая неиспользованная HGT row инвалидируют document.

Каждый initial finding review schema v4 неизменно связывает exact
`finding_id`/category/dimension и reviewer-specific tuple
`(decision, severity, hard_fail, mandatory_review)` с mapping/blinding
provenance. Disagreement — различие любого элемента tuple, а не только
`confirmed/dismissed`. Top-level finding `severity`, `hard_fail` и
`mandatory_review` обязаны byte/typed-exact совпасть с единственным matching
initial outcome либо outcome distinct third-human adjudication. Missing outcome,
self-asserted downgrade и flattening `medium` против `high` отклоняются до
acceptance; model reviewer не получает human credit.
...
approval. `not_observed` допустим только при `terminal_status=controlled_failure`
либо в primary no-attempt row с `terminal_status=not_applicable`; оба состояния
требуют пустые `observed_atoms`, D1–D5 `not_evaluated` и полностью запрещают
findings, human/model finding reviews и `human_ground_truth` для этой row.
Technical aggregate считает success только при наличии output и
`technical_conformance=synthetic_conformant`, поэтому no-attempt row никогда не
является technical либо human-quality pass. Controlled failure остаётся в
denominator; canonical no-attempt row остаётся explicit structural observation
и declared coverage, но не создаёт success. И synthetic diagnostic-scope
materialization, и будущая full-admission decision materialization учитывают
любую no-output row, включая controlled failure и no-attempt, как applicable
failure для D1–D5, CFA gate и editorial approval, поэтому её нельзя скрыть
исключением из quality denominator. Synthetic результат остаётся
diagnostic/ineligible, а production decision требует отдельного full admission.
В отдельный CFA event-rate denominator no-output row не входит: отсутствующий
output не является безопасной exposure. Для любой
row каждое D2–D5 значение `human_pass`,
...
content review, получает human ground truth по D2–D5, даже если будущая
production policy считает его class auto-eligible. Исключение —
`self_identifying_output`: он получает D2–D5=`blinding_failed`, zero primary
success, primary HGT запрещён, а optional `secondary_unblinded` evidence остаётся
только descriptive, не восстанавливает success и не входит в primary
agreement/quality gate. Для mandatory-human class
нужен предусмотренный policy human gate; model-review его не удовлетворяет.
Critical-risk class получает две разные initial human identities только для
output, прошедшего primary blinding и пригодного для blinded review; описанное
выше self-identifying exception не получает primary HGT. Reviewer ID
