...
Review и merge owner-freeze PR дают authority выбранному record только в exact
declarative scope; Git SHA остаётся только provenance.
...
Stored next-stage value — umbrella label declarative preparation scope. Он не
является owner authorization для M1B-1A1 или создания candidate.
...
Исторический `OWNER_DECISION_REQUIRED` снят только для exact declarative freeze,
описанного этим record. Отдельный exact
`EXECUTABLE_TCB_OWNER_DECISION_REQUIRED` не снят. Любой последующий
