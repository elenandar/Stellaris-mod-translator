...
До split каждый sample получает локальный `source_unit_cluster` по общей
...
Все clusters одного private `source_mod_generation` внутри одного primary
stratum сворачиваются в одну statistical unit. Поэтому один mod/source может
дать максимум одно `n` на candidate/profile/gate/stratum, даже если содержит
много файлов, events или context blocks. Ambiguous common lineage объединяется
conservative либо даёт split blocker.

Один cluster даёт максимум одну Bernoulli unit на candidate/profile/gate и один
primary stratum. Несколько rows внутри cluster агрегируются conservative: успех
только если успешны все applicable rows. Secondary risk labels не увеличивают
`n`.
...
allocation — по `36` independent clusters каждого stratum, всего `288` на каждый
candidate/profile; все три кандидата получают один и тот же frozen manifest.
Exact derivation, confidence behavior и дополнительный minimum `149` на каждый
предлагаемый auto-eligible class заданы в
...
opaque ID. Каждый holdout output получает human D2–D5 ground truth, включая
будущие auto-eligible classes; critical-risk получает две разные initial human
identities.
...
coverage; unblinded review хранится только как secondary local evidence и не
заменяет primary blinded ground truth.
...
`source_unit_cluster_id`, stratum и risk-class rows M1B-0 проверяют только
synthetic schema/split invariants. Они не доказывают real mod lineage,
independence или достаточный `n`; до holdout эти controls должны войти в новую
