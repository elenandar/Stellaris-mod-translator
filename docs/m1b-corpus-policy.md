...
closed split provenance из frozen sample. Validator materialize-ит exact полный
row multiset из immutable sample membership и validated result/finding/HGT
evidence; HGT materializer также переносит split в derived agreement row.
Agreement, statistical и CFA decision helpers принимают только соответствующий
internal sealed provenance; caller UUID/split labels либо изменённые rows не
являются evidence. Raw helpers остаются diagnostic-only и не дают decision
eligibility. Missing/unknown split отклоняется, а mixed-split scope получает
controlled `STATISTICAL_SPLIT_MIXED` до подсчёта. Decision-grade helpers
принимают только `holdout`. Tuning aggregates хранятся и публикуются отдельно,
явно имеют diagnostic status и никогда не удовлетворяют holdout minimum,
confidence bound, agreement либо verdict gate.
...
success. Decision statistical materialization сохраняет любую no-output row как
applicable failure во всех quality/gate scopes и не добавляет её в CFA event-rate
denominator; controlled error никогда не echo-ит input, content или path.
