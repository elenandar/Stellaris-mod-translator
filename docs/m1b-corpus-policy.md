...
closed split provenance из frozen sample; HGT materializer также переносит этот
split в derived agreement row. Missing/unknown split отклоняется, а mixed-split
scope получает controlled `STATISTICAL_SPLIT_MIXED` до подсчёта. Decision-grade
helpers принимают только `holdout`. Tuning aggregates хранятся и публикуются
отдельно, явно имеют diagnostic status и никогда не удовлетворяют holdout
minimum, confidence bound, agreement либо verdict gate.
...
human ground truth. Они остаются denominator failures/not-observations с D1–D5
`not_evaluated`; controlled error никогда не echo-ит input, content или path.

M1B-0 v4 sample schema включает `source_generation_id`,
