...
- Глубоко рекурсивный malformed `manifest.json` нормализуется только на границе
  `json.loads()`: `RecursionError` означает incomplete candidate, candidate root
  остаётся byte-identical, а повторный build даёт controlled
  `INCOMPLETE_BUILD_PRESENT`. Иные programming errors по-прежнему выходят наружу.
...
| `python3 -m unittest discover -s tools/research/tests -v` | `73/73 passed` |
...
| Deep malformed-manifest reproduction | before: raw `RecursionError` from state/build; after: state `incomplete`, build `INCOMPLETE_BUILD_PRESENT`; 4 000 manifest bytes and candidate root unchanged after both calls |
| `python3 tools/research/validate_m1a_docs.py` | passed; 26 Markdown files, 15 fenced blocks, 38 tables, 41 relative links, 0 errors |
...
`artifacts/m1a/m1a-local-redacted-evidence-v2-2026-07-19-pr4-recursion-followup-final.json`.
...
Repository denominator вырос с `43` до `45` без удаления старых ignored
...
один файл. Состав на момент scan — `33` tracked files, `11` retained files под
`artifacts/` и существующий ignored `.DS_Store`. Новый final JSON создаётся
только из уже проверенного path-free output после scan и потому в этот census
не входит.
...
| `tools/research/m1a_harness.py` | `c777e78de00ec896b31d0c6455e91ed69f22f4da805ad77fcbfd743ddf65620a` |
...
| `tools/research/tests/test_m1a_harness.py` | `977dd177fcebaf2fa65e797795bc764b6ff329a156e80c5aadb34aea5cf4b147` |
