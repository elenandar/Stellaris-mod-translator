...
| `M1B-T13` | Reviewer узнаёт candidate либо reviewer-specific severity/hard-fail/mandatory outcome сплющивается в self-asserted top-level finding | High | global reviewer exposure, positive mapping generations, exact finding/category/dimension provenance; immutable closed initial outcome tuple; disagreement по любому элементу требует distinct third human; randomized presentation и private mapping | external leak сохраняет compromised records и допускает только fresh never-unblinded reviewers/new mapping; missing outcome, top-level downgrade и unresolved disagreement fail closed; self-identification даёт `BLINDING_FAILED`, primary HGT forbidden, secondary evidence only |
