...
| `M1B-T28` | Stable-pair kappa смешивает reviewers, replay-ит logical rows, создаёт synthetic cross-row pairs, перевзвешивает sources с большим числом rows, включает final adjudicator ratings либо скрывает undefined/applicability disagreement | High | unique `(result_id,dimension)`, two frozen initial IDs, canonical stable distinct pair per stratum/dimension, equal-source contingency `O_ij=sum_s(count_sij/m_s)` только из actual pairs, unilateral NA blocker, exact rational formula и delete-one-whole-source gate | duplicate logical row, mixed scope/pair, noncanonical pair, missing adjudication, unilateral NA, `n<46`, undefined point/robustness либо kappa `<3/5` fail closed |
