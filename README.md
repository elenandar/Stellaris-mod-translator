...
Review и merge PR #8 зафиксировали только M1B-1A0 и не разрешили создание
candidate. Подготовленный отдельный M1B-1A1-AUTH имеет
`acceptance_state=owner_accepted`, но его effect — только
`after_review_and_merge_to_main`. Поэтому текущие states:
`M1B-1A1-AUTH: READY_FOR_OWNER_REVIEW` и
