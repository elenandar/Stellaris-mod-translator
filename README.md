...
После принятия `M0R` разрешены только два доказательных этапа: исследование реального формата и загрузки модов (`M1A`, сейчас `BLOCKED`) и изолированный benchmark качества локальных моделей (`M1B`). Exact proposal v7/generation 108 принят отдельным external owner-freeze record только как declarative basis записанного `M1B-1A local synthetic provider preflight`; после merge PR #6 exact scope действует со state `OWNER_FREEZE: ACCEPTED`, merge PR #7 выставил `STABLE_READ_HARDENING: ACCEPTED`, merge PR #8 — `M1B-1A0 CONTRACT: ACCEPTED/MERGED`, а merge PR #9 — `M1B-1A1-AUTH: ACCEPTED/MERGED`. PR #10 был слит внешним owner-controlled действием вопреки собственному stop rule; классификация — `OWNER_CONTROLLED_SCOPE_DEVIATION`; `SCOPE_V1: NEVER_EFFECTIVE`, `SCOPE_V2: SUPERSEDED_BEFORE_EFFECT`, `SCOPE_V3: SUPERSEDED_BEFORE_EFFECT`, `SCOPE_V4: SUPERSEDED_BEFORE_EFFECT`, `SCOPE_V5: SUPERSEDED_BEFORE_EFFECT`, `SCOPE_V6: SUPERSEDED_BEFORE_EFFECT`, `SCOPE_V7: SUPERSEDED_BEFORE_EFFECT`, `SCOPE_V8: SUPERSEDED_BEFORE_EFFECT`, `M1B-1A-R1-AUTH-V9: SUPERSEDED_BEFORE_EFFECT`, `M1B-1A-R1-AUTH-V10: SUPERSEDED_BEFORE_EFFECT`, и ни одна из них не разрешает это действие retroactively. Evidence v8, v9 и v10 не создавались. V10 остановился до первого child spawn и до evidence write boundary из-за несовместимых helper mode-полей; отдельно его Python publisher содержал stale parent-launcher SHA, unbound wrapper маскировал child failure, а builder расходился с publisher по publication mechanism. Слитые candidate bytes остаются inert и reviewable: candidate source не принят как executable, import/parse/tokenize/lint/compile/execution не разрешены, runtime envelope, invocation instance и acceptance records не создавались, executable admission не выдан, provider/Ollama/benchmark не запускались. Текущая граница: `M1B-1A-R1-AUTH-V11-DEFINITION: REVIEWABLE`, `POSTPUBLICATION_EVIDENCE: REQUIRED`, `TERMINAL_PUBLICATION_RECEIPT: REQUIRED`, `PR11_METADATA_RECONCILIATION: REQUIRED`, `EFFECT: NOT_ACTIVE`, `R1_REMEDIATION: NOT_AUTHORIZED_UNTIL_V11_MERGE`, `NEW_REPOSITORY_CODE_EXECUTION: NOT_AUTHORIZED`, `EXECUTABLE_TCB_ADMISSION: NOT_GRANTED`, `M1B-1A PROVIDER EXECUTION: NOT_STARTED`, `M1B: NOT_EVALUATED`. Только принятые verdicts `M1A: GO` и `M1B: QUALITY_FEASIBLE` вместе разрешают `M2`; сейчас `M2: FORBIDDEN`, массовый перевод и active publish запрещены.
...
- [Post-merge transport/provenance remediation scope M1B-1A-R1-AUTH v11](registry/m1b/m1b-1a-r1-remediation-scope-v11.json)
...
- [Machine owner authorization M1B-1A-R1-AUTH v11](docs/decisions/M1B-1A-R1-AUTH-owner-authorization.json)
- [Owner signoff M1B-1A-R1-AUTH v11](docs/decisions/M1B-1A-R1-AUTH-owner-signoff.md)
...
Отдельный
[M1B-1A-R1-AUTH v11 scope](registry/m1b/m1b-1a-r1-remediation-scope-v11.json)
...
`SCOPE_V5: SUPERSEDED_BEFORE_EFFECT`,
...
создавались. V11 разделяет Git tree/index modes `100644` и POSIX worktree
modes `0644`/`0600`, закрывает matrix из `32` actions, `106` resources,
`7` planes, `66` normative process definitions и `4` external operations.
Новая action/resource pair описывает обязательный end-to-end no-write
preflight; terminal publication во всей цепочке имеет literal
`fclonefileat_open_source_fd_CLONE_NOFOLLOW_EXCL`.
No-write preflight запускает bound implementation-coupled suite: один live
positive parent-auth, семь parent-auth/write-boundary negatives и двенадцать
child-diagnostics/strict-receipt/privacy negatives. PASS receipts проверяются
по всем exact types/domains; reviewer IDs закрыты двумя public opaque
значениями. Его sanitized FAILURE receipt явно разрешён, но staging/target
writes запрещены. Неизвестное после сбоя состояние обозначается literal
`UNKNOWN`, а не выдуманным boolean. Zero-status publish отмечает
write boundary до post-child reads; lost/malformed stdout допускает только
exact bound final inspection, которая остаётся последним filesystem read перед
in-memory receipt validation.
GitHub publication допускает ровно один title/body `PATCH` существующего PR
#11 между двумя authenticated `GET`: precondition и обязательным readback.
При потере ответа `PATCH` разрешён только readback `GET`.

Текущие states: `M1B-1A-R1-AUTH-V11-DEFINITION: REVIEWABLE`,
`POSTPUBLICATION_EVIDENCE: REQUIRED`,
`TERMINAL_PUBLICATION_RECEIPT: REQUIRED`,
`PR11_METADATA_RECONCILIATION: REQUIRED`, `EFFECT: NOT_ACTIVE` и
