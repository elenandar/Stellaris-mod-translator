...
- [Post-merge transport/provenance remediation scope M1B-1A-R1-AUTH v10](registry/m1b/m1b-1a-r1-remediation-scope-v10.json)
...
- [Machine owner authorization M1B-1A-R1-AUTH v10](docs/decisions/M1B-1A-R1-AUTH-owner-authorization.json)
- [Owner signoff M1B-1A-R1-AUTH v10](docs/decisions/M1B-1A-R1-AUTH-owner-signoff.md)
...
Отдельный
[M1B-1A-R1-AUTH v10 scope](registry/m1b/m1b-1a-r1-remediation-scope-v10.json)
...
создавались. V10 разделяет Git tree/index modes `100644` и POSIX worktree
modes `0644`/`0600`, закрывает matrix из `31` actions, `105` resources,
...
#11 и один authenticated `GET` для reconciliation; при потере ответа `PATCH`
разрешён только этот `GET`.

Текущие states: `M1B-1A-R1-AUTH-V10-DEFINITION: REVIEWABLE`,
